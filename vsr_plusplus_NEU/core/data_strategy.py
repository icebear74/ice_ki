...
class DataStrategyScheduler:
    ...
    Phase 2 – Crop Introduction (steps 5000–15000):
        Data : linear interpolation from 100 % 720_169 → approximate natural
               distribution (CROP_INTRO_END_DISTRIBUTION), so that by step
               15000 the override values closely match what is already on disk.
               step 5000  → {720_169: 1.00, 540: 0.00, 720: 0.00}
               step ~15000 → approaching {720_169: 0.25, 540: 0.50, 720: 0.25}
        Loss : perceptual weight 0.0 → 0.05 (linear)

    Phase 3 – Stable Training (steps 15000+):
